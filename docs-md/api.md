...
С помощью этих методов сервер принимает сырые или частично агрегированные данные на первичную обработку и сохранение. Для получения данных для отображения на графиках или анализа используйте [API хранилища](api-ag.md).
...
Если следовать концепции [SSSO](./ssso.md), то на самом деле нам нужно как минимум три счетчика:
```json
{"method":"push", "params":{"name":"script~~MyScript~~MyService~~MyOp", "data":[200]}}
...
Если вы используете концепцию [SSSO](./ssso.md), то имена счетчиков могут составлять большую часть в отправленных сообщениях. Их можно сократить, например:
...
Аналогично [get](#get), но для нескольких графиков
...
* остальные параметры см. [get](#get)
